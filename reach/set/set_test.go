...
func TestNewSetFromSingleValue(t *testing.T) {
	cases := []struct {
		name  string
		value uint16
	}{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			s := NewSetFromSingleValue(tc.value)
...
func TestString(t *testing.T) {
	cases := []struct {
		...
		{
			"single value (middle)",
			NewSetFromSingleValue(12),
			"12",
		},
		{
			"upper bound (single value)",
			NewSetFromSingleValue(65535),
			"65535",
		},
		{
			"lower bound (single value)",
			NewSetFromSingleValue(0),
			"0",
		},
		{
			"upper + lower bound (single values)",
			NewSetFromSingleValue(0).merge(NewSetFromSingleValue(65535)),
			"0, 65535",
		},
	}
